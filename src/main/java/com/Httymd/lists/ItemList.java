package com.Httymd.lists;

import net.minecraft.item.Item;

public class ItemList {
	//Items
	public static Item gronckle_iron;
	public static Item gronckle_iron_nugget;
	
	//Blocks
	public static Item mud_block;
	public static Item loki_sapling;
	public static Item loki_log;
	public static Item loki_plank;
	public static Item loki_stair;
	public static Item loki_slab;
	public static Item loki_fence;
	public static Item loki_door;
	public static Item loki_leaves;
	public static Item pine_sapling;
	public static Item pine_log;
	public static Item pine_plank;
	public static Item pine_stair;
	public static Item pine_slab;
	public static Item pine_fence;
	public static Item pine_door;
	public static Item pine_leaves;
	
	//Gronckle Iron Tools
	public static Item gronckle_iron_axe;
	public static Item gronckle_iron_hoe;
	public static Item gronckle_iron_pickaxe;
	public static Item gronckle_iron_shovel;
	
	//Gronckle Iron Melee Weapons
	public static Item gronckle_iron_sword;
	public static Item gronckle_iron_dagger;
	public static Item gronckle_iron_warhammer;
	public static Item gronckle_iron_battleaxe;
	public static Item gronckle_iron_club;
	public static Item gronckle_iron_mace;
	
	//Default material Melee Weapons
	public static Item wood_dagger;
	public static Item stone_dagger;
	public static Item iron_dagger;
	public static Item gold_dagger;
	public static Item diamond_dagger;
	public static Item wood_battleaxe;
	public static Item stone_battleaxe;
	public static Item iron_battleaxe;
	public static Item gold_battleaxe;
	public static Item diamond_battleaxe;
	public static Item wood_club;
	public static Item stone_club;
	public static Item iron_club;
	public static Item gold_club;
	public static Item diamond_club;
	//public static Item wood_mace;  apparently not in use
	//public static Item stone_mace; apparently not in use
	public static Item iron_mace;
	public static Item gold_mace;
	public static Item diamond_mace;
	
	//Rnaged Gronckle Iron Weapons
	public static Item gronckle_iron_spear;
	
	//Ranged Weapons
	public static Item wood_spear;
	public static Item stone_spear;
	public static Item iron_spear;
	public static Item gold_spear;
	public static Item diamond_spear;
	
	//Armor
	public static Item gronckle_iron_helmet;
	public static Item gronckle_iron_chestplate;
	public static Item gronckle_iron_leggings;
	public static Item gronckle_iron_boots;
	
	public static Item terrible_terror_egg;
	public static Item yak_egg;
	public static Item boar_egg;
	public static Item eel_egg;
}
